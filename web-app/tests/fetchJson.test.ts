...
describe('fetchJson', () => {
  it('caches successful responses', async () => {
    ...
    const first = await fetchJson<number>('u', cache, ledger, j => j as number);
    const second = await fetchJson<number>('u', cache, ledger, j => j as number);
    expect(first).toBe(3);
  ...
  it('respects quota ledger', async () => {
    ...
    global.fetch = vi.fn();
    const res = await fetchJson<number>('u', cache, ledger, j => j as number);
    expect(res).toBeNull();
  ...
  it('does not cache failed requests', async () => {
    ...
    await fetchJson<number>('u', cache, ledger, j => j as number);
